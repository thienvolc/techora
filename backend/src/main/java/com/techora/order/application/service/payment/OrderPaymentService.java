...
public class OrderPaymentService {
    ...
    public PayableOrderSnapshot preparePayment(UUID userId, UUID orderId) {
        Order order = orderPermissionService.getLockedOwnedOrderOrThrow(userId, orderId);
        if (order.getStatus() == OrderStatus.PAYMENT_PENDING) {
            return toPayableOrderSnapshot(order);
        }

        OrderStatusChange statusChange = updatePaymentPendingStatus(order);
        return toPayableOrderSnapshot(statusChange.order());
    }
    ...
    private PayableOrderSnapshot toPayableOrderSnapshot(Order order) {
        return PayableOrderSnapshot.from(order);
    }
